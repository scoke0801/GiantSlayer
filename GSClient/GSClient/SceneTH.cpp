...
void CSceneTH::LoadTextures(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	auto fbxtestTex = make_unique<CTexture>();
	MakeTexture(pd3dDevice, pd3dCommandList, fbxtestTex.get(), "FbxTest", L"resources/OBJ/grey.dds");
	...
	m_Textures[fbxtestTex->m_Name] = std::move(fbxtestTex);
}
...
void CSceneTH::BuildDescripotrHeaps(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	auto fbxtestTex = m_Textures["FbxTest"]->m_pd3dResource;
	...
	hDescriptor.ptr += gnCbvSrvDescriptorIncrementSize;
	srvDesc.Format = fbxtestTex->GetDesc().Format;
	pd3dDevice->CreateShaderResourceView(fbxtestTex, &srvDesc, hDescriptor);
}
