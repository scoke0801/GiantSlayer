...
void CSceneTH::Init(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, int width, int height)
{
	...
	CShaderHandler::GetInstance().SetUserID(ShaderHandlerUser::TH); // 수정필요
...
void CSceneTH::BuildCamera(ID3D12Device* pd3dDevice,
...
void CSceneTH::UpdateForMultiplay(float elapsedTime)
{
	m_SoundManager->OnUpdate();
	ProcessInput();
...
void CSceneTH::DrawMirror(ID3D12GraphicsCommandList* pd3dCommandList, ID3D12Resource* pd3dRTV,int idx)
{
	pd3dCommandList->SetGraphicsRootSignature(m_pd3dGraphicsRootSignature);
...
void CSceneTH::DrawShadow(ID3D12GraphicsCommandList* pd3dCommandList)
{
...
void CSceneTH::ProcessPacket(unsigned char* p_buf)
{
	char buf[10000];
...
void CSceneTH::CreateLightCamera(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, int nWidth, int nHeight)
{
...
void CSceneTH::BuildParticles(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
...
void CSceneTH::BuildMummyLaser(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	m_Mummy[0] = new CMummy();
	for (int i = 0; i < 3; i++)
	{
...
void CSceneTH::ShotMummyLaser(CMummy* pMummy, const XMFLOAT3& lookVector)
{
...
void CSceneTH::EnterNewSector(int sectorNum)
{
...
void CSceneTH::ShotMonsterArrow(CEnemy* pEmeny, const XMFLOAT3& lookVector)
{
...
void CSceneTH::DeleteEnemy(CEnemy* pEmeny)
{
	int i = 0;
...
void CSceneTH::UseEffects(int effectType, const XMFLOAT3& xmf3Position)
{
...
void CSceneTH::UseEffects(int effectType, const XMFLOAT3& xmf3Position, float wakeupTime)
{
	auto effect = m_EffectsHandler->RecycleEffect((EffectTypes)effectType);
	if (effect != nullptr) {
		effect->SetPosition(xmf3Position);
...
void CSceneTH::SendMouseInputPacket()
{
...
void CSceneTH::BuildBoundingRegions(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	// 4개 벽 테두리
...
void CSceneTH::RecvMouseProcessPacket()
{
	return;
	int retVal;
