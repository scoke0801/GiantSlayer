...
#include "State.h"
#include "SceneYJ.h"
CMummy::CMummy() : CGameObjectVer2()
{
	m_Type = OBJ_TYPE::Mummy;

	//m_HeightFromTerrain = 150.0f; 
	m_State = new PatrolState_M(this);
...
void CMummy::UpdateOnServer(float fTimeElapsed)
{
...
bool CMummy::IsMummyInSight() // Chase State
{
	if (m_AttackDelayTime > 0.0f) {
		return false;
	}
	for (auto player : m_ConnectedPlayers) {
		...
		for (auto plAABB : plAABBs) {
			if (ret = m_SightAABB.Intersects(plAABB->GetBox())) {
				...
				//m_AttackDelayTime = MELLE_Mummy_ATTACK_TIME + 1.5f; 
...
void CMummy::SetActivityScope(const XMFLOAT3& xmf3ActivityScope, const XMFLOAT3& xmf3Center)
{
...
void CMummy::SetSightBoundingBox(const XMFLOAT3& sight)
{
...
void CMummy::ConnectPlayer(CPlayer** pPlayers, int playerCount)
{
...
void CMummy::ChangeState(CState<CMummy>* nextState)
{
	m_State->Exit(this);
	delete m_State;
	m_State = nextState;
...
void CMummy::FindClosePositionToTarget()
{
...
void CMummy::MoveToNextPosition(float elapsedTime)
{
	const float Mummy_SPEED_TEMP = 165.0f * 2.5f;
	SetPosition(Vector3::Add(m_xmf3Position, Vector3::Multifly(m_xmf3Velocity, Mummy_SPEED_TEMP * elapsedTime)));
	m_ToMovePosition.y = m_xmf3Position.y;
...
void CMummy::LookTarget(bool rotatedModel)
{
	XMFLOAT3 playerPos = m_TargetPlayer->GetPosition();
	playerPos.y = m_xmf3Position.y;
...
void CMummy::FixCollision(CGameObject* pCollideObject)
{
...
void CMummy::ChangeState(ObjectState stateInfo, void* pData)
{
	switch (stateInfo)
	{
	...
	case ObjectState::Attack:
		ChangeState(new AttackState_M(this));
		break;
	case ObjectState::Attacked:
		ChangeState(new AttackedState_M(this));
		break;
...
CRangedMummy::CRangedMummy()
{
	m_Type = OBJ_TYPE::Mummy;
	m_AttackType = MummyAttackType::Ranged;
...
void CRangedMummy::Attack(float elapsedTime)
{
	// 공격관련 애니메이션 수행
	// 현재는 임시코드
	{
		float rotateAnglePerFrame = 360.0f / LASER_MUMMY_ATTACK_TIME;
	...
	if (m_AttackDelayTime <= 0.0f) {
		// 실제 공격!
		cout << "원거리 몬스터 화살 발사\n";
		m_AttackDelayTime = LASER_MUMMY_ATTACK_TIME + 1.0f;
...
CMeleeMummy::CMeleeMummy()
{
	m_Type = OBJ_TYPE::Mummy;
	m_AttackType = MummyAttackType::Melee;
...
void CMeleeMummy::Attack(float elapsedTime)
{
	// 공격관련 애니메이션 수행
	// 현재는 임시코드
	{
	...
	if (m_AttackDelayTime <= 0.0f) {
		// 실제 공격!  
		m_AttackDelayTime = MELLE_MUMMY_ATTACK_TIME + 1.0f;
	}
}

void CMeleeMummy::FindNextPosition()
{
	m_ToMovePosition.x = (((float)rand() / (RAND_MAX)) * (m_xmf3ActivityScope.x * 2)) + m_xmf3ActivityScopeCenter.x - m_xmf3ActivityScope.x;
	m_ToMovePosition.y = m_xmf3Position.y;
	m_ToMovePosition.z = (((float)rand() / (RAND_MAX)) * (m_xmf3ActivityScope.z * 2)) + m_xmf3ActivityScopeCenter.z - m_xmf3ActivityScope.z;
...
void CMeleeMummy::FindClosePositionToTarget()
{
	XMFLOAT3 playerPos = m_TargetPlayer->GetPosition();
	playerPos.y = m_xmf3Position.y;
